...
def test_post_todos(auth_client, test_todo_one):
    response = auth_client.post("/api/todos", json=test_todo_one)
...
def test_put_todos_not_found(auth_client, non_existing_user, test_todo_one):
    response = auth_client.put(f"/api/todos/{non_existing_user}", json=test_todo_one)
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "Todo not found"}
